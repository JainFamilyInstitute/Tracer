...
#     start = time.time()
#
#     income = adj_income_process(income_bf_ret, sigma_perm, sigma_tran, principal, ppt_bar, n_sim, path=base_path)
#
#     ###########################################################################
#     #                    DP - read consumption functions                      #
#     ###########################################################################
    ...
    for fp in cfunc_fps:
        ...
        if int(ppt_bar) in debt_dict:
            ...
            for ap, i in zip(agent_pctl, np.arange(4) + 1):
                ...
                agent_agj_inc = adj_income[allowed_rows]

                c_proc, _ = generate_consumption_process(agent_agj_inc, c_func_df, agent_agj_inc.shape[0])
                # op: Consumption
                agent_c = c_proc
...
#             agent_util = util
#             single_op.append(c_ce)
#             ipcu_df.loc[i] = np.squeeze(np.array([agent_inc, agent_c, agent_util]))
#
#         print(f'########## Gamma: {ppt_bar} | Gamma: {gamma} | Exp_Frac: {gamma_exp_frac[gamma]} | CE: {c_ce:.2f} ##########')
#         print(f"------ {time.time() - start} seconds ------")
#         op.append(single_op)
#         ipcu_df.to_csv(os.path.join(base_path, 'results', f'cg_quartile_IPCU_gamma_{gamma}.csv'))
#     df = pd.DataFrame(op)
#     df.to_csv(os.path.join(base_path, 'results', 'cg_quartile_CEs.csv'))
#
#
# param_pair = [None, None]
# run_model(param_pair, income_bf_ret, sigma_perm, sigma_tran, surv_prob, base_path, 10000)
...
#         print(f"------ {time.time() - start} seconds ------")
#         op.append(single_op)
#         ipcu_df.to_csv(os.path.join(base_path, 'results', f'hsg_quartile_IPCU_gamma_{gamma}.csv'))
#     df = pd.DataFrame(op)
#     df.to_csv(os.path.join(base_path, 'results', 'hsg_quartile_CEs.csv'))
#
#
# param_pair = [None, None]
# run_model(param_pair, income_bf_ret, sigma_perm, sigma_tran, surv_prob, base_path, 10000)
